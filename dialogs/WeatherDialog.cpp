...
, m_webpage         {nullptr}
{
  ...
  connect(this, &QDialog::destroyed, parent, [this]() { m_webpage = nullptr; });
  setupUi(this);
  ...
  weatherTabLayout->setContentsMargins(0, 0, 0, 0);
  ...
  pollutionTabLayout->setContentsMargins(0, 0, 0, 0);
  ...
  uvTabLayout->setContentsMargins(0, 0, 0, 0);
  ...
  m_webpage->settings()->setAttribute(QWebEngineSettings::Accelerated2dCanvasEnabled, true);
  m_webpage->settings()->setAttribute(QWebEngineSettings::JavascriptEnabled, true);
  m_webpage->settings()->setAttribute(QWebEngineSettings::JavascriptCanOpenWindows, false);
...
void WeatherDialog::setWeatherData(const ForecastData &current, const Forecast &data, Configuration &config)
{
  ...
  else
  {
    ...
    const auto theme = m_config->lightTheme ? QChart::ChartThemeQt : QChart::ChartThemeDark;
        ...
        case Representation::BARS:
          {
            ...
            auto line = new QBarSeries(forecastChart);
            line->setName(name);
            line->append(barset);
            line->setUseOpenGL(true);
            line->setOpacity(0.8);
...
void WeatherDialog::updateMapLayerValues()
{
  if(m_webpage != nullptr && m_webpage->property("finished").toBool())
  {
    QVariant value;
	QEventLoop loop;
	m_webpage->page()->runJavaScript("customGetLayer();", [&](const QVariant &res) {
    	value = res;
    	loop.quit();
	});
	loop.exec();
    ...
    QEventLoop loop2;
	m_webpage->page()->runJavaScript("customGetStreet();", [&](const QVariant &res) {
    	value = res;
    	loop2.quit();
	});
	loop2.exec();
...
//--------------------------------------------------------------------
void WeatherDialog::updateAxesRanges(QChart *chart)
{
