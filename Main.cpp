...
int main(int argc, char *argv[])
{
  ...
  // allow only one instance - vylepšená a odolná kontrola vůči pádům
  QSharedMemory guard;
  ...
  if (!guard.create(1))
  {
    // Pokud vytvoření selhalo, zkusíme se k paměti připojit.
    // Na Linuxu: pokud se připojit NEDOKÁŽEME, znamená to, že aplikace předtím spadla
    // a systém nechal paměť viset jako sirotka.
    if (!guard.attach())
    {
        // Paměť sice existuje, ale nikdo ji nevlastní -> bezpečně ji uvolníme
        // a vytvoříme znovu pro tuto naši novou instanci.
        guard.detach(); 
        guard.create(1);
    }
    else
    {
        // Pokud attach() USPĚL, znamená to, že jiná instance TrayWeather OPRAVDU legálně běží.
        guard.detach(); // odpojíme se, abychom neblokovali
