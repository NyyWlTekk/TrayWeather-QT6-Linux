...
void TrayWeather::showConfiguration()
{
  ...
  const auto scr = QGuiApplication::primaryScreen()->geometry();
...
void TrayWeather::showTab()
{
  ...
  // Vracíme původní Felixovo volání, které kompilátor zná a bez problémů sestaví
  m_weatherDialog = new WeatherDialog(m_provider);
  m_weatherDialog->setAttribute(Qt::WA_DeleteOnClose);
  
  // TENTO NOVÝ ŘÁDEK vyčistí Chromium mapy z RAM v momentě, kdy se okno zavře a Felix zavolá 'delete'
  m_weatherDialog->setAttribute(Qt::WA_DeleteOnClose);
  connect(m_weatherDialog, &QObject::destroyed, this, [this]() {
    m_weatherDialog = nullptr;
});
